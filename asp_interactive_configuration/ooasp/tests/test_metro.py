...
def test_metro_cv_wagon():
    ...
    for i in range(20):
        ...
        nr_p = list(c.fb.query(attr).where(attr.attr_name == 'nr_passengers').select(
            attr.attr_value).all())[0].symbol.number
        nr_s = list(c.fb.query(attr).where(attr.attr_name == 'nr_seats').select(attr.attr_value).all())[0].symbol.number
        sr = list(c.fb.query(attr).where(attr.attr_name == 'standing_room').select(
            attr.attr_value).all())[0].symbol.number
        assert nr_s >= 0
        assert nr_s <= 20
        assert sr >= 0
