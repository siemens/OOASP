...
class OOASPUI:
    ...
    def __init__(self, iconf):
        ...
        """
        self.iconf = iconf
        self.config_image = widgets.Image(format='png', width='1000px', layout=Layout(
            overflow='visible', min_width='1000px', height='300px'), unconfined=True)
        self.found_config = widgets.Image(format='png', width='1000px', layout=Layout(
            overflow='visible', min_width='1000px', height='300px'), unconfined=True)
        ...
        display(
            HTML("<style>div.output_scroll { height: 44em; } .container { width:100% !important; }</style>"), self.grid)
    ...
    def set_extend(self):
        ...
        self.extend.children = tuple([self.title('Extend'), HBox(
            children=[domain_lbl, extend_domain]), config_lbl, dropdown, dropdown_propagate])
    ...
    def set_browse(self):
        ...
        incremental = Button(description='Find incrementally', button_style=colora,
                             disabled=self.invalid, layout=b_layout)
        incremental.on_click(self.call_and_update(self.button_wrapper('extend_incrementally')))
        incrementalos = Button(description='Find incrementally (overshoot)',
                               button_style=colora, disabled=self.invalid, layout=b_layout)
        ...
        self.browse.children = tuple([self.title('Browse'), incremental, incrementalos,
                                     next_solution, select_found, end_browsing])
