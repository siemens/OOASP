...
class OOASPKnowledgeBase:
    ...
    def associations(self, class_name: str) -> List:
        ...
        left = set(self.fb.query(Assoc).where(Assoc.class1 == class_name).select(
            Assoc.name, Assoc.class2, Assoc.min2, Assoc.max2).all())
        right = set(self.fb.query(Assoc).where(Assoc.class2 == class_name).select(
            Assoc.name, Assoc.class1, Assoc.min1, Assoc.max1).all())
        assocs.update(left)
        assocs.update(right)
        return assocs
