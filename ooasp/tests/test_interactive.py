...
def test_s_interactive_ccheck():
    ...
    assert (
        'ooasp_cv(lowerbound,2,"Lowerbound for {} is {} but has {} {}",(rack_frames,1,0,rack'
        in iconf.state.config.fb.asp_str()
    )
    assert (
        'ooasp_cv(lowerbound,1,"Lowerbound for {} is {} but has {} {}",(rack_frames,1,0,rack'
        in iconf.state.config.fb.asp_str()
    )
    assert (
        'ooasp_cv(lowerbound,3,"Lowerbound for {} is {} but has {} {}",(rack_frames,1,0,rack'
        in iconf.state.config.fb.asp_str()
    )
    assert (
        'ooasp_cv(no_value,1,"Missing value for {}",(frame_position,))'
        in iconf.state.config.fb.asp_str()
    )
    assert (
        'ooasp_cv(no_value,2,"Missing value for {}",(frame_position,))'
        in iconf.state.config.fb.asp_str()
    )
    assert (
        'ooasp_cv(no_value,3,"Missing value for {}",(frame_position,))'
        in iconf.state.config.fb.asp_str()
    )
...
def test_s_interactive_check_custom_cv():
    ...
    iconf.check()
    assert (
        '(upperbound,1,"Upperbound for {} exceeded: {}",(rack_framesS,4,'
        in iconf.state.config.fb.asp_str()
    )
...
def test_not_a_subclass():
    ...
    iconf.select_object_class(1, "rackDouble")
    ok = iconf.check()
    # Its ok because the rackDouble is ignored since it no external is grounded for user(ooasp_isa(1,rackDouble))
    assert ok
...
def test_incremental_adding_multiple_racks():
    ...
    assert len(added) == 1  # One rack and three frames
